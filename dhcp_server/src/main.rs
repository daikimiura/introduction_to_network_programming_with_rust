...
fn main() {
    ...
    loop {
        ...
        match server_socket.recv_from(&mut recv_buf) {
            ...
            Err(e) => {
                error!("Cloud not receive a datagram: {}", e);
            }
        }
    }
}
...
fn dhcp_discover_message_handler(
    xid: u32,
    dhcp_sever: Arc<DhcpServer>,
    received_packet: &DhcpPacket,
    soc: &UdpSocket,
) -> Result<(), failure::Error> {
    info!("{:x}: received DHCPDISCOVER", xid);

    let ip_to_be_leased = select_lease_ip(&dhcp_sever, &received_packet)?;
    let dhcp_packet = make_dhcp_packet(&received_packet, &dhcp_sever, DHCPOFFER, ip_to_be_leased)?;
...
fn obtain_available_ip_from_requested_option(
    ...
    let requested_ip = util::u8_to_ipv4_addr(&ip)?;
...
fn dhcp_request_message_handler_responded_to_offer(
    ...
    let server_ip = util::u8_to_ipv4_addr(&server_id)
    ...
    let ip_bin = received_packet
        ...
        .unwrap();
    let ip_to_be_leased = util::u8_to_ipv4_addr(&ip_bin)
        .ok_or_else(|| failure::err_msg("Failed to convert ip addr"))?;
    ...
    debug!("{:x}: leased adderss: {}", xid, ip_to_be_leased);
    match count {
...
fn dhcp_request_message_handler_to_reallocate(
    ...
    if let Some(requested_ip) = received_packet.get_option(Code::RequestedIpAddress as u8) {
        ...
        let requested_ip = util::u8_to_ipv4_addr(&requested_ip)
            .ok_or_else(|| failure::err_msg("Failed to convert ip addr"))?;
        let con = dhcp_server.db_connection.lock().unwrap();
