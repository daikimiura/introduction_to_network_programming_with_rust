...
impl DhcpPacket {
    ...
    pub fn get_option(&self, option_code: u8) -> Option<Vec<u8>> {
        let mut index: usize = 4; // skip magic cookie
...
impl DhcpServer {
    pub fn new() -> Result<DhcpServer, failure::Error> {
        ...
        let lease_time = util::make_big_endien_vec_from_u32(
            env.get("LEASE_TIME").expect("Missing lease time").parse()?,
        )?;
    ...
    fn init_address_pool(
        ...
        let mut address_pool: Vec<Ipv4Addr> = network_addr_with_prefix
            .iter()
            .filter(|addr| !used_ip_addrs.contains(addr))
            .collect();

        address_pool.reverse();
        Ok(address_pool)
    }
